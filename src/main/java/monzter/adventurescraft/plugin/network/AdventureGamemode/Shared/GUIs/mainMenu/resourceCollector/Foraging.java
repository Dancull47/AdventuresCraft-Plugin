...
public class Foraging extends BaseCommand {
    ...
    public final List<ItemStack> FORAGING_MATERIALS;

    public Foraging(AdventuresCraft plugin, SoundManager soundManager, GUIHelper guiHelper, ConsoleCommand consoleCommand, Pickup pickup, BetonPointsManager betonPointsManager, PermissionLP permissionLP, MMOItems mmoItems, ItemAdder itemAdder) {
        this.plugin = plugin;
        ...
        FORAGING_MATERIALS = Arrays.asList(new ItemStack(Material.OAK_LOG), new ItemStack(Material.SPRUCE_LOG), new ItemStack(Material.DARK_OAK_LOG), new ItemStack(Material.BIRCH_LOG), new ItemStack(Material.ACACIA_LOG), new ItemStack(Material.JUNGLE_LOG),
                new ItemStack(Material.HONEYCOMB));
    }


    @CommandAlias("ForagingResources")
    private void createMenu(Player player) {
        final ChestGui gui = new ChestGui(4, "Foraging Resources");
        ...
        for (ItemStack item : FORAGING_MATERIALS) {
            if (i < 28) {
    ...
    private GuiItem itemCreator(Player player, ItemStack itemStack) {
        ...
        return new GuiItem(forest, e -> {
            ...
            player.performCommand("ForagingResources");
        });
    }
