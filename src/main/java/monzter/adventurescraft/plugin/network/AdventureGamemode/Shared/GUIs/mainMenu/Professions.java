...
public class Professions extends BaseCommand {
    ...
    private ItemStack farming(Player player) {
        profession = "Farming";
        int level = Integer.valueOf(parsePlaceholder(player, "mmocore_profession_" + profession));
        ...
        return guiHelper.itemCreator(getMaterial(level, "HOE"), name, new String[]{
                ...
                progressBar.getProgressBar(Integer.valueOf(parsePlaceholder(player, "mmocore_profession_experience_" + profession)), Integer.valueOf(parsePlaceholder(player, "mmocore_profession_next_level_" + profession)), 13, '-', ChatColor.YELLOW, ChatColor.GRAY) + " " + ChatColor.GOLD + parsePlaceholder(player, "mmocore_profession_percent_" + profession) + "%",
                "",
    ...
    private ItemStack fishing(Player player) {
        profession = "Fishing";
        int level = Integer.valueOf(parsePlaceholder(player, "mmocore_profession_" + profession));
        String name = ChatColor.GREEN + profession + " Level " + ChatColor.GOLD + level;
        return guiHelper.itemCreator(Material.FISHING_ROD, name, new String[]{ChatColor.GRAY + "Coming Soon..."});
    }
